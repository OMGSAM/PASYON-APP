...
def ajouter():
    ...
    #Creeon la connexion
    con = sqlite3.connect("sbitar")
    cuser = con.cursor()
    cuser.execute("insert into kliyan('matricule','nom','prenom','age','adresse','telephone','remarque') values (?,?,?,?,?,?,?)",(matricule,nom,prenom,age,adresse,telephone,remarque))
    con.commit()
    con.close()
    ...
    con = sqlite3.connect("sbitar")
    cuser = con.cursor()
    select = cuser.execute("select *from kliyan order by code desc")
    select = list(select)
...
def modifier():
        ...
        con.commit()
        con.close()
        messagebox.showinfo("Patient Modifié succesfully")
...
#Ajouter le titre
lbltitre = Label(root,bd = 20, relief = RIDGE, text = "MY HOSPITAL", font = ("Arial", 30), bg = "orange", fg="black")
lbltitre.place(x = 0, y = 0, width = 1365)

#Liste des patients
lblListePatient = Label(root, text = "LISTES DES PATIENTS ", font = ("Arial", 16), bg = "orange", fg="black")
...
#Enregistrer
btnenregistrer = Button(root, text = "Enregistrer", font = ("Arial", 16),bg = "orange", fg = "black", command = ajouter)
btnenregistrer.place(x=30, y= 450, width=200)

#modifier
btnmodofier = Button(root, text = "Modifier", font = ("Arial", 16),bg = "orange", fg = "black", command = modifier)
btnmodofier.place(x=270, y= 450, width=200)

#Supprimer
btnSupprimer = Button(root, text = "Supprimer", font = ("Arial", 16),bg = "orange", fg = "black", command = supprimer)
...
con =  sqlite3.connect("sbitar")
cuser = con.cursor()
select = cuser.execute("select * from kliyan")
for row in select:
